...
**Multi-unit path (built):** `LearningPath.tsx` renders every unit in order as a continuous gated trail. A unit computes LIVE availability (`unitAvailability`): a theme with lessons opens once the learner clears its `unlockAfter` unit's review accuracy (`unlockAccuracy`, default 0.7); themes with no lessons show "coming soon". Lessons carry `subUnit` (rendered as signpost chips) and `milestone` (hex node). Daily Hex distractors only pull from vocab the learner has a card for (no forward-referencing).

**Units built:** Greetings (6 lessons) and **Family & Home** (9 lessons across 4 sub-units — Immediate Family, Extended Family, Around the House, Chores & Home Life — plus a milestone; grammar woven in: `mei` possessive, `sell is`, `wer/was is sell?`, `Kind→Kinner` plural). Content in `/content/vocab/family-home.json` + `/content/lessons/family-home-1..9.json`, all `needsReview`.

**Remaining themes** (Farm & Animals, Numbers & Time, Food, Weather & Seasons, Market Day, Faith & Community, Barn Raising) are `coming_soon` stubs in `content/units.ts` with the unlock chain pre-wired — build one theme per session following the Family & Home pattern (vocab file → lesson files → register in `lib/content.ts` → set lessonIds). All authored content is AI-drafted and MUST be native/heritage-speaker reviewed before wide release (PRD §10).
